...
def function_uncode(df: pd.DataFrame, colName=None, mapping=None) -> pd.DataFrame:
  ...
  mapping = mapping or {}
  if colName is None:
    # columns = [list(set(mapping.keys()) | COLUMN_DISABILITY)]
...
def plotly_variance_heatmap(
    ...
    fig.update_layout(
        ...
        xaxis=dict(categoryorder='array', categoryarray=ordered_cols),  # Explicit x-axis ordering
...
def plotly_stack_bar(
    ...
    for col in columns:
        ...
        for category in counts.index:
            ...
            # ------------------------------------------
            # Best-fit label orientation
            # ------------------------------------------
            #
            # Large segment  -> horizontal
            # Small segment  -> vertical
            #
...
def function_sankey_cascade_log(dataframe,criteria_dict,title="TB Cascade of Care",width=1000,height=500,log_base=10):
    ...
    fig = go.Figure(
        go.Sankey(
            arrangement="snap",
            node=dict(pad=30,thickness=35,label=labels,color=node_colors,line=dict(color="black",width=1)),
            link=dict(source=source,target=target,value=values,customdata=np.column_stack((original,percentages)),
                hovertemplate=
                "<b>%{source.label}</b>"
                "<br>↓<br>"
                "<b>%{target.label}</b>"
    ...
    fig.update_layout(title=dict(text=title,x=0.5),width=width,height=height,template="plotly_white",dragmode="zoom",margin=dict(l=30,r=30,t=70,b=30))
